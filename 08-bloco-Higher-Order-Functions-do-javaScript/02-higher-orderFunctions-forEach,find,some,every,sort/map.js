...
// transforme o array de objetos citis , em um array de string s de um seguinte formato
// usando forEach:   depois usando map
// const res = cities.map((city)=>{
//   return `${city.name} - ${city.state}`
// });
// console.table(res);

const res1 = cities.map((city) => {
  const estado = states.find((uf) => {
    uf.short === city.state;
    return uf
  });
  return `A Cidade de ${city.name} fica no estado de ${estado.name}`;
});
console.table(res1);
